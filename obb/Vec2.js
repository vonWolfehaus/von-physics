...
Vec2.prototype.rotateBy = function(theta, axis) {
...
Vec2.prototype.rotateByAxis = function(theta, axis) {
	var x = this.x - axis.x, y = this.y - axis.y;
	var cos = Math.cos(theta), sin = Math.sin(theta);
	this.x = axis.x + (x * cos) - (y * sin);
	this.y = axis.y + (x * sin) + (y * cos);
	return this;
};
...
Vec2.prototype.getLeftNormal = function() {
	var x = -this.x;
	this.x = this.y;
	this.y = x;
	return this;
...
Vec2.prototype.getRightNormal = function() {
	var y = -this.y;
	this.y = this.x;
	this.x = y;
	return this;
...
Vec2.unitVector = function(deg) {
	return new Vec2(Math.cos(Math.PI * deg / 180), Math.sin(Math.PI * deg / 180));
};